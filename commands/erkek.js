...
module.exports.run = async (client, message, args) => {
 ...
 message.guild.members.cache.get(uye.id).roles.add(cfg.erkek)
 message.guild.members.cache.get(uye.id).roles.remove(cfg.kayıtsız)
  data.add(`kayıt.erkek_${message.author.id}`, 1)
...
.setDescription(`${uye} isimli kullanıcı <@&${cfg.erkek}> olarak kaydedildi`))
...
name: "e",
guildOnly: true,
aliases: ["erkek", "man", "men", "body", "adam"],
};
